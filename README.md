...
```
 ...
 │ Chess.com   │─────┐
 │ Public API  │     │      ┌────────────────┐          ┌─────────────────┐
 └─────────────┘     ├─────▶│ Game Harvester │─────────▶│     SQLite      │
 ┌─────────────┐     │      │ Multi-platform │          │    Database     │
 │  Lichess    │─────┘      │ + Deduplicator │          │                 │
 │ Public API  │            └────────────────┘          │  • games        │
 └─────────────┘                                        │  • move_analysis│
 ┌─────────────┐                                        │  • coaching     │
 │    FIDE     │─── rating lookup ──────────────────────│  • patterns     │
 │  Database   │                                        │  • players      │
 └─────────────┘                                        └───────┬─────────┘
                                                                │
                    ANALYSIS                                    │
                    ════════                                    │
                                                                │
                    ┌────────────────┐                          │
                    │ Stockfish 17   │◀──── PGN replay ────────┤
                    │ Depth 18–28    │                          │
                    │ (per tier)     │── per-move evals ───────▶│
                    └────────────────┘                          │
                                                                │
                    ┌────────────────┐                          │
                    │  LLM Coaching  │◀──── analysis data ─────┘
                    │                │
                    │ Claude Opus 4.6│── narrative + feedback ──▶ DB
                    │   GPT-5.4     │
                    └────────────────┘
               ...
               │  Dashboard   │    │   Reports      │
               │              │    │                │
               │ • Player hub │    │ • Weekly MD    │
               │ • Game replay│    │ • Monthly MD   │
               │ • Live coach │    │ • Pattern      │
               │ • ACPL trend │    │   summaries    │
               │ • Light/Dark │    │                │
               └──────────────┘    └────────────────┘
```

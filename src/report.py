...
def build_report_data(player_slug: str, period: str = "weekly",
                      db_path: str | None = None) -> dict:
    """Build structured report data for a player by slug.

    v1.16.4: slug-only lookup. chess.com username is harvester-only
    since v1.16.4 — the previous v1.16.3 backward-compat fallback
    was removed once the slug feature stabilised. The positional
    arg name reflects this: `player_slug`, not the older
    `player_identifier` / `player_username`.
    ...
    player = conn.execute(
        "SELECT * FROM players WHERE slug = ?", (player_slug,)
    ).fetchone()
    if not player:
        conn.close()
        raise ValueError(f"Player {player_slug} not found")
    ...
    display_name = player["display_name"] or player_slug
